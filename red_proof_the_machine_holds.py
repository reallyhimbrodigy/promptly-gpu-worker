...
MUTATIONS = [
    # ── PRESTAGE AT ATTACH ──
    # Each mutation makes the PROPERTY false, never the file invalid: a red
    # that is not about the property is exactly as wrong as a green that is
    # not, and this lane has produced two of those from mutating a guard into
    # a crash rather than mutating the answer.
    ('the claim reads instead of taking, so two jobs share one project',
     '        rec = store.pop(key)',
     '        rec = store[key]',
     'a claim REMOVES the record, so a second job on the same source misses',
     'store.pop(key)'),
    ('a key part stops reaching the digest',
     '             "w": w, "h": h, "fps": fps}',
     '             "w": w, "h": h}',
     'every key part changes the key — a part that does not is a collision waiting',
     '"fps": fps}'),
    ('a missing key part is hashed as None instead of refused',
     '    missing = sorted(k for k, v in parts.items() if v is None or v == "")',
     '    missing = []',
     "a key with any part missing is REFUSED, never hashed as the string 'None'",
     'if v is None'),
    ('an unreadable timeline passes as an empty one',
     '    if items is None:\n        return dict(out, state="STALE", checked=out["checked"] + ["timeline"],',
     '    if False:\n        return dict(out, state="STALE", checked=out["checked"] + ["timeline"],',
     'an unreadable read-back says so rather than passing as an empty timeline',
     '    if items is None:'),
    ("a timeline holding another job's items still verifies",
     '    if base_only and extra:',
     '    if False and extra:',
     'a verified record is MEASURED only when the timeline holds the base clip and nothing else',
     'if base_only and extra:'),
    ('the sheets are trusted from the record instead of opened',
     '        if not os.path.exists(p):',
     '        if False:',
     'sheets are read off the volume — an absent or wrong-sized one is STALE, not a warm hit',
     'if not os.path.exists(p):'),
    ('a missing reader passes as a verified project',
     '    if reader is None:\n        return dict(out, state="FAILED", checked=out["checked"],',
     '    if reader is None:\n        return dict(out, state="MEASURED", checked=out["checked"],',
     'a verify with no reader is FAILED — an unknown project is not a verified one',
     '    if reader is None:'),
    ("a job's own titles are silently dropped into a warm project",
     '    if titles:\n        return dict(out, why="this job prestages',
     '    if False:\n        return dict(out, why="this job prestages',
     'a job prestaging its own titles or controls goes COLD and says why',
     '    if titles:'),
    ('the saving is quoted as the setup cost rather than the gap',
     '    saved = min(gap, cold)',
     '    saved = cold',
     'the saving is min(gap, setup) and is ABSENT when either input is missing',
     'saved = min(gap, cold)'),
    ('a 0-byte download becomes a valid key',
     '    if n == 0:',
     '    if True is False:',
     'a 0-byte source is FAILED, not a valid hash every failed download would share',
     '    if n == 0:'),
    ...
    ('a native caption track stops being a fault',
     '    if native:\n        return {"state": "MEASURED", "fault": True,',
     '    if False:\n        return {"state": "MEASURED", "fault": True,',
     'a native caption item or native cards is a FAULT, and a clean timeline is not',
     '    if native:'),
    ('an unread caption state reads as off',
     '    if st != "MEASURED":\n        return {"state": st if st in ("ABSENT", "FAILED") else "ABSENT", "fault": None,',
     '    if False:\n        return {"state": st if st in ("ABSENT", "FAILED") else "ABSENT", "fault": None,',
     'an unreadable caption state is UNKNOWN, never read as off',
     'if st != "MEASURED":'),
    ('an unknown caption style is substituted instead of refused',
     '    if style not in CAPTION_STYLES:',
     '    if False:',
     'an unknown style is REFUSED, never substituted, and no text is ABSENT',
     'if style not in CAPTION_STYLES:'),
    ("the caption tool goes back on the agent's surface",
     'AGENT_TOOLS = ["edit_item", "finish"]',
     'AGENT_TOOLS = ["edit_item", "edit_captions", "finish"]',
     "edit_captions is not in the agent's tool list, and the fault is at the withholding seam",
     'AGENT_TOOLS = ["edit_item", "finish"]'),
