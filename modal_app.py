...
# rebuild trigger v38 — Fix 400 INVALID_ARGUMENT from Google on main edit call. Google's structured-output subset rejected the dynamically-injected enum schema (likely anyOf-with-enum or enum-size issue). Root fix: instead of enum-constraining the schema, RE-INDEX the transcript so Gemini sees only kept words [0..M-1]. The index space itself is the structural guarantee — cut words don't exist in it, so anchors physically cannot reference them. Base EditPlan.model_json_schema() is used (no enum injection). After Gemini returns, Python translates every anchor field's new_index → source_index. Anchor-integrity guard drops any Gemini range cut covering an anchored word (anchor wins). All v37 wins preserved: mechanical pre-pass, context-aware analysis call, graceful degradation, parallelization.
