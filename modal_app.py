...
# rebuild trigger v55 — Vulkan startup probe. After tracing the per-frame paint cost (44 mixBlendMode + 53 CSS filter usages introduced by the 66-component pack vs. 0 in the pre-pack era), confirmed that the bottleneck is NOT Chromium-can't-use-GPU but rather software-painted blend modes at 1080x1920 are ~200ms each, and we stack 7+ per frame for typical NeoNoir+NegativeFlash workloads. Quality is non-negotiable — the only quality-preserving fix is hardware GPU paint. v54 flipped gl to vulkan but hasn't been render-tested yet. This v55 build adds three OS-level Vulkan diagnostics at handler.py startup: (1) Vulkan ICD JSONs registered in /etc/vulkan/icd.d (proves NVIDIA ICD is registered); (2) presence of libGLX_nvidia / libnvidia-egl / libnvidia-vulkan in /usr/local/nvidia/lib (proves Modal's nvidia-container-toolkit mounted them); (3) vulkaninfo --summary filtered to physical device lines (proves Vulkan can enumerate the H100). Together with render-full.mjs's WebGL renderer probe, these pinpoint exactly where Vulkan breaks if it does — OS layer, ICD registration, or Chromium binding. No behavior change; pure instrumentation.
