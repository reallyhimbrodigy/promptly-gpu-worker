...
# rebuild trigger v53 — Remove hardwareAcceleration='required' from renderMedia. v52 hit the loud failure I asked for, just from a different validation: Remotion threw 'crf option is not supported with hardware acceleration'. Misread of Remotion's API on my part — hardwareAcceleration controls the ENCODER (NVENC vs libx264), not Chromium's GL backend. H100 has no NVENC ASIC (Modal startup log: 'NVENC not available (H100/A100 lack encode ASIC) — using CPU encoder'), so 'required' is structurally impossible on this hardware. The actual Chromium GPU paint path is fully controlled by chromiumOptions.gl='vulkan' (kept), which adds --enable-gpu / --ignore-gpu-blocklist / --use-angle=vulkan / --use-vulkan=native at browser launch — that engages the H100 for per-frame paint. Encode stays libx264 ultrafast, which is fine: v51 measured the encoder catching up at 65 fps, so it was never the bottleneck. v52's other fixes (gl='vulkan', WebGL probe via Page.goto, libvulkan1+vulkan-tools in image) all stay. Expected: WebGL probe will print the renderer string for the first time (NVIDIA H100 vs SwiftShader Google Inc.), and if Vulkan engages, render drops from ~150s to ~25-40s.
