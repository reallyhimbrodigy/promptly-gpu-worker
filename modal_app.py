...
# rebuild trigger v57 — Fix the two bugs that kept v56's NVIDIA Vulkan ICD registration from taking. v56's filesystem walk DID find Modal's NVIDIA graphics libs (32 of them at /usr/lib/x86_64-linux-gnu/) and DID synthesize /etc/vulkan/icd.d/nvidia_icd.json. But vulkaninfo still saw only llvmpipe afterwards — meaning the loader never honored our registration. Two concrete bugs: (1) Set VK_DRIVER_FILES (modern, requires Vulkan loader >= 1.3.207) but Ubuntu 22.04 ships an older loader that only honors the legacy VK_ICD_FILENAMES env var. The loader silently ignored VK_DRIVER_FILES, fell back to default search, and llvmpipe won. Fix: set BOTH vars, covers any loader version. (2) The ICD JSON pointed at the absolute path libGLX_nvidia.so.580.95.05 — but NVIDIA's driver internally dlopens sub-libs by soname (libGLX_nvidia.so.0), and Modal only mounts the full versioned filename, no soname symlinks. The existing soname-symlink loop in handler.py:749 only matched libnvidia-* and libcuda.so.* — it never created libGLX_nvidia.so.0 or libEGL_nvidia.so.0 symlinks. Fix: extend the symlink loop to also match libGLX_* and libEGL_* prefixes; switch ICD JSON to use the soname libGLX_nvidia.so.0 so ld.so.cache resolves it. Together these should let vulkaninfo enumerate the H100, Chromium gl='vulkan' bind to the NVIDIA Vulkan ICD, and the per-frame mixBlendMode passes drop from ~200ms each to ~1ms — 200x speedup on the actual paint bottleneck. Zero component changes; quality preserved.
