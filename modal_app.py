...
# rebuild trigger v41 — Fix Remotion 404 once and for all by staging local files into bundle/public/ at runtime. The publicDir parameter doesn't actually extend Remotion's bundle-server serving roots in practice (verified via Remotion docs + v40 404 logs). The documented pattern is: physical files inside the bundle's public/ subfolder are served at /public/<name> URLs. Implementation: render_multi_clip creates /remotion/bundle/public/<stage_key>/ (per-job to avoid warm-container collisions), symlinks work_dir's source_30fps.mp4 + broll files into it, passes paths like "<stage_key>/source_30fps.mp4" via the input JSON so PromptlyRender's staticFile() wrap produces /public/<stage_key>/<file> URLs. After render, try/finally rmtree's the stage dir. Symlinks are zero-copy. Uses Remotion's own static server (HTTP Range, parallel concurrency, all proven). Drops --public-dir flag from render command (now unused).
