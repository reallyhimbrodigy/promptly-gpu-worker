...
# rebuild trigger v56 — NVIDIA Vulkan ICD auto-registration attempt. v55 diagnostics revealed the actual block: Modal isn't mounting NVIDIA's graphics drivers despite NVIDIA_DRIVER_CAPABILITIES='all'. The libs aren't at /usr/local/nvidia/lib*, no nvidia_icd.json is registered in /etc/vulkan/icd.d/, and vulkaninfo --summary sees only llvmpipe (Mesa CPU software). chrome-for-testing wouldn't help — same Modal mount problem. v56 attempts a focused fix: filesystem-wide search for libGLX_nvidia.so / libEGL_nvidia.so / libnvidia-vulkan / libnvidia-glcore at startup; if found anywhere on disk, synthesize /etc/vulkan/icd.d/nvidia_icd.json pointing at it and set VK_DRIVER_FILES so the Vulkan loader picks it up. Re-runs vulkaninfo afterwards to verify the H100 became visible. Two outcomes: (a) NVIDIA libs are mounted at a non-standard path → ICD registration succeeds, vulkaninfo lists the H100, render speed restored without quality loss; (b) NVIDIA libs are genuinely absent → confirms Modal-platform-level limitation, hard escalation needed (different GPU type, alternate render arch, or platform). One-shot diagnostic-and-fix; no component changes.
