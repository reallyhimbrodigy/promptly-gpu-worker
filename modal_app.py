...
# rebuild trigger v49 — Render-speed root fix. Diagnostic logging (v47-diag) revealed every video frame seek was taking 1.6-2.3s — even consecutive frames. Cause: source_30fps.mp4 was encoded with x264's default keyframe interval (~250 frames / 8s GOP), so every Remotion seek required ffmpeg to decode hundreds of frames forward from the previous keyframe to reach the target. With 1340 output frames × 32 concurrent tabs, that seek cost dominated the entire 152s render. Fix: re-encode source_30fps.mp4 with -g 30 -keyint_min 30 -sc_threshold 0 (1 keyframe per second). Worst-case seek now decodes 30 frames forward (~50ms) instead of 240 (~2000ms). File size grows ~30% but render time should drop from ~150s to ~30-40s. Also: dropped Remotion logLevel from verbose to info (cuts thousands of compositor stream lines), filtered stderr echo to only keep error/warning/seek-time signals so we don't blow up the operator log on every render.
