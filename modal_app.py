...
# rebuild trigger v54 — Actually flip gl to vulkan. v53 deployed but the v53 render still showed gl=angle-egl in the log: handler.py:8287 hardcoded `_gl_mode = "angle-egl" if _HAS_HWACCEL else "swiftshader"`, and that CLI arg overrides render-full.mjs's default that I'd flipped. The Vulkan path was never exercised — render still 139s @ 9 fps avg, GPU util 0%, and the WebGL probe came back "no WebGL context available" (angle-egl can't even create a WebGL context in this headless image, which is consistent with SwiftShader fallback throughout). v54 changes one line in handler.py: angle-egl → vulkan when _HAS_HWACCEL. Vulkan is the only Remotion gl mode that adds --enable-gpu / --ignore-gpu-blocklist / --use-angle=vulkan / --use-vulkan=native at browser launch (see open-browser.js:getOpenGlRenderer()). NVIDIA driver has first-class Vulkan support mounted via NVIDIA_DRIVER_CAPABILITIES='all', and v52's libvulkan1 + vulkan-tools are already in the image. Expected: WebGL probe prints actual renderer string for the first time (NVIDIA H100 vs SwiftShader), and if Vulkan engages, render drops from ~140s to ~25-40s.
