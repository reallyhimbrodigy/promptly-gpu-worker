...
# rebuild trigger v50 — Cross-field validators converted to drop-and-continue. Replaced 11 hard-fail ValueError raises in handler.py validate_edit_plan() with the established drop-component-and-log pattern (mirrors the transitions validator from v44): InvertStrike↔mode mismatch (drops color_effect), motion_graphics start/end_word_index targeting removed words (drops MG), emphasis_moments.word_indices[k] targeting removed words (drops emphasis), high-intensity emphasis pacing <2.5s (drops the second), zoom_effect outside any clip (clears just the zoom, keeps emphasis), duplicate zoom_effect on same clip (clears second zoom), text_overlays.start_word_index targeting removed words (drops overlay), text_overlay vs text_overlay zone collisions (drops second), text_overlay vs emphasis MG zone collision (drops overlay), sound_effects.word_index targeting removed words (drops SFX). Whole-plan crashes from any single Gemini cross-field inconsistency are now eliminated — render proceeds with the rest of the plan and a loud [generate-edit] DROP/CLEAR log line for the operator.
