...
# rebuild trigger v42 — Zone-aware overlap validation. Old validator hard-failed on any text_overlay + text_overlay time-window overlap or text_overlay + emphasis-MG time-window overlap, rejecting valid pro layouts (e.g. torn_paper at center + lower_third at bottom). Replaced with zone-aware checks: two overlays in DIFFERENT visual zones freely coexist at the same time; only same-zone + same-time is a real collision. Per-variant text_overlay zones: torn_paper→center, sticky_note→upper_third_safe, quote_card→center, lower_third→lower_third_safe, caption_match→matches its position. motion_graphic zone = its anchor field. Updated Contract Rule #7, verification footer Rule #1, and per-variant TEXT OVERLAYS prompt sections to document zones explicitly so Gemini self-constrains. Same class of structural prevention as every other word-anchored root fix; no buffer, no post-processing.
