...
# rebuild trigger v40 — Fix Remotion 404 on local-file sources. The composition was using `<OffthreadVideo src={sourceUrl}>` with bare basenames ("source_30fps.mp4"), which Remotion's bundle HTTP server tried to resolve against the prebundle dir (/remotion/bundle) and 404'd because the file lives in publicDir (work_dir). Root fix: wrap every basename URL with staticFile() at the boundary in PromptlyRender. staticFile() resolves the path against publicDir, returning a URL the bundle server serves correctly. Single resolution point at the top-level component covers all consumers (clip renderer, transitions, broll, zoom components) — every child gets an already-resolved URL. Absolute URLs (https://, http://, file://) pass through unchanged. Bundle rebuild required since the Remotion composition changed.
