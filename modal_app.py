...
image = (
    ...
    .run_commands(
        "echo 'build v39 - transition symmetric handles: fix clip-B-side backward-jump glitch in L-cut transitions. Previously clip B render and transition B-side BOTH started at source[start_B], so clip Bs first 0.4s of source rendered twice (transition with effect + render at full opacity) — viewer saw a rewind glitch at every transition seam. Now clipBStartFromFrames is shifted backward by trans_dur*pbrB so the transitions B-side covers source[start_B - trans_dur*pbrB, start_B] (the inter-clip silent gap) and clip Bs render continues seamlessly from source[start_B]. Each source frame rendered exactly once. PRECONDITION GUARD: drop transitions where inter-clip gap < trans_dur*pbrB (revert to hard cut). Prompt updated: transitions require ≥0.5s inter-clip gap.'",
