...
# rebuild trigger v39 — ALL timing is word-anchored. Eliminated every float-timestamp-to-word-boundary-match path that could hard-fail. Migrated 5 fields from float timestamps to word indices: caption_position_segments→caption_position_changes, hook_clip.source_start/end→start/end_word_index, thumbnail_timestamp→thumbnail_word_index, color_effect.pulses[].peak_at_seconds→peak_word_index, speed_curve[].t→at_word_index. Gemini emits word indices only; Python synthesizes all float timestamps in a derivation pass after the main call. Caption segments are SYNTHESIZED (guaranteed to align on word boundaries), not emitted. Deleted obsolete boundary-match validator. Downstream render code unchanged (reads derived fields). All v38 wins preserved: pre-pass, analysis, re-indexing, translation, parallelization.
