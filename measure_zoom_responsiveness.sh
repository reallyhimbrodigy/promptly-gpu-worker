...
set -uo pipefail
URL="${1:?usage: measure_zoom_responsiveness.sh <url-or-path> [label]}"
LABEL="${2:-source}"
T=$(mktemp -d)
# One second, mid-clip, at native scale vs the same second zoomed 1.10x centre.
ffmpeg -hide_banner -loglevel error -y -ss 5 -t 1 -i "$URL" -vf "scale=540:960" "$T/a.mp4" 2>/dev/null
ffmpeg -hide_banner -loglevel error -y -ss 5 -t 1 -i "$URL" \
