...
MUTATIONS = [
    ...
    ("a reason covering a refused family is no longer flagged",
     '            if _lost:', '            if False:',
     "must be flagged", None),
    ("every ruled family is treated as refused — the flag cries wolf",
     '            _lost = [f for f in _ruled_f\n'
     '                     if f not in ("none", "keep", "cut") and f not in _built_f]',
     '            _lost = [f for f in _ruled_f if f not in ("none", "keep", "cut")]',
     "must NOT be flagged", None),
