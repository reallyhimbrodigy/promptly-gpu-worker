...
def generate_subtitle_file(transcript, caption_style, cuts, effective_durations, output_res, caption_position, caption_keywords, work_dir, hook_offset=0.0, hook_clip=None, speed_curve=None, transition_duration=None, face_positions=None, edit_plan=None):
    ...
    STYLE_CONFIGS = {
        ...
        "captions_clean": ("&H00FFFFFF", "&H00FFCC00", "&H80000000", 1, 0, 3, 0),
        "captions_dynamic": ("&H00FFFFFF", "&H00FFCC00", "&H80000000", 1, 0, 3, 0),
    }
...
"""
    ...
    if caption_style == "keyword_pop":
        ...
        for i, word_dict in enumerate(words):
            ...
            pause = (next_w["start"] - word_dict["end"]) if next_w else 1.0
            ends_sentence = bool(PUNCT_END.search(word_dict.get("word") or ""))
            if not next_w or pause > 0.25 or ends_sentence or len(group) >= 4:
        ...
        ass += "".join(ass_lines)
        print(f"[captions] Captions Dynamic style: {len(ass_lines)} groups, {len(keyword_set)} keywords, kw_fs={_kw_fs}", flush=True)
...
# ─── REMOTION VIDEO OVERLAY RENDERER ──────────────────────────────────────────
# Full visual overlay engine: animated captions + transitions + particles +
# light leaks + glitch effects + emoji pops + progress bar — all rendered
# in one pass as a transparent ProRes 4444 MOV via Remotion (React + Chromium).
...
def render_remotion_overlay(
    ...
    """Render captions + visual effects as a transparent overlay video via Remotion.
    ...
    _n_words = len(words) if words else 0
    print(f"[remotion] Rendering: {caption_style} ({_n_words} words), "
          f"{len(cut_points)} cuts, {len(em_list)} emphasis, vibe=\"{vibe[:50]}\", "
          f"{total_duration:.1f}s @ {fps}fps", flush=True)
    t0 = time.time()
    ...
    for _gl_mode in ["angle-egl", "swiftshader"]:
        ...
        if _gl_mode == "angle-egl":
            print(f"[remotion] GPU GL failed, retrying with swiftshader...", flush=True)
            continue

        # Both GL modes failed
        full_stderr = result.stderr or ""
        if full_stderr:
            for line in full_stderr.strip().split("\n")[-20:]:
                print(f"  {line}", flush=True)
        raise RuntimeError(f"[remotion] Render failed (rc={result.returncode})")
    ...
    sz = os.path.getsize(output_mov_path)
    print(f"[remotion] Overlay: {sz / 1024 / 1024:.1f}MB in {elapsed:.1f}s "
          f"(concurrency={_concurrency}, gl={_gl_mode})", flush=True)
    return output_mov_path
