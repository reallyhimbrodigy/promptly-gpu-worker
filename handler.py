...
def format_trend_section(trend_context):
    ...
    try:
        if isinstance(trend_context, dict) and trend_context.get("type") == "style_guide":
            ...
            # Strip broll_clips from the style guide — the example keywords
            # override our b-roll instructions and make Gemini produce
            # narrative-matching keywords instead of stock footage titles.
            import re as _re_trend
            _cleaned_guide = _re_trend.sub(
                r'"broll_clips"\s*:\s*\[.*?\]',
                '"broll_clips": []',
                style_guide,
                flags=_re_trend.DOTALL,
            )
