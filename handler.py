...
def render_multi_clip(source_path, cuts, edit_plan, output_path, transcript, work_dir, speech_segments=None,
    ...
    # ── PASS 1: Concat without captions (runs immediately — no Remotion wait) ──
    ...
    # Pass 1 uses fast intermediate encode (CRF 10 ultrafast) — visually lossless
    _n_threads = os.cpu_count() or 32
    _intermediate_path = output_path.replace(".mp4", "_intermediate.mp4")
    _pass1_encode = [
    ...
    print(f"[render] PASS 1 (no captions): {n} segments, ~{running_dur:.1f}s output", flush=True)
    _t_pass1 = time.time()
    run_ffmpeg(args_pass1)
    print(f"[render] Pass 1 done in {time.time() - _t_pass1:.1f}s", flush=True)

    # ── Wait for Remotion overlay (should already be done or nearly done) ──
    ...
    # ── PASS 2: Overlay captions onto intermediate → final output ──
    if caption_overlay_path:
        _t_pass2 = time.time()
        # Remotion rendered at half-res (540x960) — upscale to full 1080x1920 with lanczos
        _pass2_filter = (
            "[1:v]scale=1080:1920:flags=lanczos,format=yuva420p[_cap_alpha];"
        ...
        args_pass2 = (
            ["-y", "-i", _intermediate_path,
             "-c:v", "libvpx", "-i", caption_overlay_path,
             "-filter_complex_threads", str(_n_threads),
             "-filter_complex", _pass2_filter,
             "-map", "[vout]", "-map", "0:a"]
            + ["-threads", str(_n_threads)]
            + encode_args
            + [output_path]
        )
        print(f"[render] PASS 2 (caption overlay): upscale 540x960 → 1080x1920", flush=True)
        run_ffmpeg(args_pass2)
        # Clean up intermediate
        try:
            os.remove(_intermediate_path)
        except OSError:
            pass
        print(f"[render] Pass 2 done in {time.time() - _t_pass2:.1f}s", flush=True)
    else:
        # No captions — just rename intermediate to final
        import shutil
        shutil.move(_intermediate_path, output_path)
        print("[render] No caption overlay — using pass 1 output directly", flush=True)
