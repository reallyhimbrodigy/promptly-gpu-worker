...
**The spine is everything.** Before you touch components you commit to what THIS video is, where the hook, payoff, and close land, and how the arc tiles between them. Every component choice flows from that spine. A choice that doesn't trace to the arc is a rule firing, not an edit.

**Specificity over genre.** A B-roll keyword that could fit ANY pitch video doesn't fit THIS pitch video. Reach into the dialogue's actual details — the named thing, the specific moment, the specific texture — and anchor your choices there.
...
The proxy attached to this call is the actual source video — 480p, 22fps, full audio. Watch it before you place anything. The transcript and signal annotations are supporting evidence, not substitutes. While you watch, read:
...
**Doubt is resolved by the timeline, not by temperament.** There is no "when in doubt, place" and no "when in doubt, skip." There is: look at the window. Already has its event → done, don't add. Empty and the dialogue offers something fitting → place it. Empty and nothing fits → leave it on the speaker.
...
**The pause that lands.** After a payoff, the speaker often holds a natural beat. Don't decorate it. A zoom that HOLDS through that silence lets the moment land; an MG or transition there steps on the moment you just earned.
...
**The `reason` field is a picker hint, not prose.** A second Gemini call sees the candidate clips' thumbnails plus the dialogue line AND this `reason` as an "editor's note" when choosing between them. Write it as a direction TO that picker: one short sentence naming the specific visual the clip must SHOW (or must NOT show) for this cutaway to land. Example for an app-input line: "must show an app's text-input field on screen, not a person holding a phone." Example for a savings beat: "must show coins or savings physically — a piggy bank, jar, or cash — not an abstract money graphic." Generic prose ("visual of the speaker's point") wastes a wire that already exists. The keyword fishes the candidate pool; the reason tells the picker how to choose within it.

**Keyword construction.** Decide which KIND of moment the dialogue is — the three modes below want different keywords.

**(1) CONCRETE / PRODUCT / DEMO.** The speaker names a specific action, object, screen, or result the viewer must literally SEE to follow the point ("upload your video," "it launched on the App Store," "every edit is tailored"). Depict THAT LITERAL THING — the app interface, the upload on a screen, the actual object. An evocative stand-in fails: for "upload your video," a person typing on a phone is a person typing on a phone, not uploading to an app, and a stock search matches surface action to the wrong scene. Name the real thing: "screen recording uploading video file to mobile app interface progress bar."

**(2) APP-INPUT (a sub-pattern of concrete — the most common failure).** When the dialogue is the user typing INTO an app — a prompt, description, query, command, or comment ("type in," "type into," "enter," "fill in," "describe," "write," "paste") — the SUBJECT of the keyword is the SCREEN or INTERFACE, never a person at a keyboard. The keyword STARTS with "screen recording," "app interface," "mobile UI," or "smartphone screen close up" — NEVER "person texting," "person typing on phone," "hands typing on keyboard," "person messaging," or any human-and-device framing. Human-action keywords return texting/messaging clips; you need the app screen showing the input being entered. For "type in the vibe": "screen recording user typing text prompt into mobile app input field interface close up." **The test:** if the keyword could equally fit a generic texting clip, sharpen until only an app-screen result could match.

**(3) NARRATIVE / ABSTRACT.** The dialogue describes a feeling, scene, approach, or story beat with no specific object the viewer must see ("the office she walked into," "the frustration of it all"). EVOKE, don't recreate the nouns: "The secretary came into my office" — not "modern office secretary typing on computer" (noun-recreation; the viewer gains nothing) but "anxious woman walking down corporate office hallway dim lighting late evening" (the approach itself). Abstract emotions ("feeling of dread") produce generic stock — always anchor on something filmable.
...
Why: The build's window gets its event from the concrete noun — B-roll on the saving while the dialogue accumulates. Keyword construction is structural: concrete noun + motion + mood, never abstractions — "vintage ceramic piggy bank coins falling" is a subject doing an action; "cinematic warm window light" gives the search engine mood. The same approach turns "the office she walked into" into "anxious woman walking down corporate office hallway dim lighting," never "the feeling of dread." The mid_peak's window gets the zoom: snap on the SECOND "THREE" because the repetition is what landed — the first instance set up the punctuation, the second IS the punctuation. Two windows, two events, no stacking.
...
def generate_edit_gemini(
    ...
    # video_metadata.fps=22 tells Gemini to SAMPLE at 22fps. Without this,
    ...
    # higher value). Paired with the 480p@22fps proxy encode (see
    # _do_gemini_proxy), this is what actually delivers "Gemini watches
    # at the model's max supported rate".
    _video_fps_meta = genai_types.VideoMetadata(fps=22) if hasattr(genai_types, "VideoMetadata") else None
    if inline_video_bytes:
        ...
        print(f"[generate-edit] Using inline video ({len(inline_video_bytes)/1024/1024:.1f}MB, no upload, sample_fps=22)", flush=True)
        ...
        print(f"[generate-edit] Using pre-uploaded Gemini file: {gemini_file.uri} (sample_fps=22)", flush=True)
    else:
...
def _prewarm_cached_proxy_path(bucket, key):
    """Pre-encoded 480p@22fps Gemini proxy. When the prewarm worker runs
...
def prewarm_handler(job):
    ...
    try:
        ...
        # Gemini proxy encode — 480p @ 22fps, matches the render-time
            ...
            try:
                ...
                _pr = subprocess.run(
                    ["ffmpeg", "-y", "-v", "error", "-threads", "0"] + _hw_dec + [
                     "-i", source_cache,
                     "-vf", "scale=480:-2,fps=22"] + _proxy_venc + [
                ...
                if _pr.returncode == 0 and os.path.exists(proxy_cache) and os.path.getsize(proxy_cache) > 1024:
                    ...
                    print(
                        f"[prewarm] proxy cached (480p@22fps, {_px_mb:.1f}MB in "
    ...
    try:
        ...
        def _do_gemini_proxy():
            ...
            Three paths, in priority order:
              ...
              3. No client proxy AND no prewarm cache — encode 480p@22fps
            ...
            try:
                ...
                # 480p @ 22fps proxy (bumped from 480p @ 10fps). Paired with
                # video_metadata.fps=22 on the Gemini Part so Gemini SAMPLES
                # at 22fps — without that metadata the SDK defaults to ~1fps
                # and bumping the encoder is performative. 24 is the API's
                # hard cap (validated 2026-06-13: fps>24 returns
                # INVALID_ARGUMENT). At 22fps the model sees micro-expression
                ...
                with open(_proxy_path, "rb") as f:
                    _proxy_bytes = f.read()
                _proxy_mb = len(_proxy_bytes) / (1024 * 1024)
                print(f"[pipeline] Gemini proxy: 480p@22fps {_proxy_mb:.1f}MB in {time.time()-_proxy_t:.1f}s (on-server encode, no client proxy)", flush=True)
                return _proxy_bytes
            except Exception as e:
