...
MUTATIONS = [
    ...
    ("the consecutive-run arm stops firing — a rolling transcript reads editorial",
     "    _sub = (_run >= min_run) or (_rest >= 3 and _share >= 0.7)",
     "    _sub = (_rest >= 3 and _share >= 0.7)",
     "CONSECUTIVE-RUN arm alone", _INJECTS),
    ("the share arm stops firing",
     "    _sub = (_run >= min_run) or (_rest >= 3 and _share >= 0.7)",
     "    _sub = (_run >= min_run)",
     "SHARE arm alone", _INJECTS),
    ("a one-word stamp starts counting as a subtitle",
     "        _marks[_v.get(\"beat\")] = _best >= min_run",
     "        _marks[_v.get(\"beat\")] = _best >= 1",
     "ONE-WORD stamp", _INJECTS),
    ("no text rulings reads clean instead of ABSENT",
     '        return {"state": "ABSENT", "n_text": 0, "n_restating": 0,',
     '        return {"state": "MEASURED", "n_text": 0, "n_restating": 0,',
     "reads ABSENT, never a clean", _INJECTS),
    ("the field stops carrying the rule the agent never reads",
     '"beat. THE CAPTIONS ALREADY "', '"beat. "',
     "says the captions already carry", _INJECTS),
    ("the subtitle track stops failing loudly",
     '        fail("overlay_is_a_second_subtitle_track",',
     '        _note("overlay_is_a_second_subtitle_track",',
     "fails LOUDLY to us", _INJECTS),
    ("it reads the rewritten record instead of the executed rulings",
     '    _ors = overlay_restates_speech(led.get("executed_verdicts")\n'
     '                                   or led.get("beat_verdicts"), led.get("beats"))',
     '    _ors = overlay_restates_speech(led.get("beat_verdicts"), led.get("beats"))',
     "reads the EXECUTED rulings first", _INJECTS),
    ("the bar stops noticing it has fallen inside one population",
     '    if _gap >= min_gap_db:\n        return ("SEPARATES", "a %.2f dB gap straddles the bar" % _gap)',
     '    if True:\n        return ("SEPARATES", "a %.2f dB gap straddles the bar" % _gap)',
     "report INSIDE_CLUSTER", _INJECTS),
    ("one value is enough to claim separation",
     '    if len(_v) < 2:\n        return ("ABSENT", "fewer than two measured deltas")',
     '    if len(_v) < 1:\n        return ("ABSENT", "fewer than two measured deltas")',
     "two points cannot show a gap", _INJECTS),
    ("an infinite delta counts as a value",
     '    _v = sorted(float(d) for d in (deltas or [])\n'
     '                if d is not None and d not in (float("inf"), float("-inf")))',
     '    _v = sorted(float(d) for d in (deltas or []) if d is not None)',
     "not counted as a value", _INJECTS),
    ("an unsupportable bar stops failing loudly",
     '            fail("inert_bar_inside_population",',
     '            _note("inert_bar_inside_population",',
     "fails LOUDLY, naming its verdicts", _INJECTS),
    ("the window-elsewhere control gets a bar again",
     '    "window_elsewhere": None,', '    "window_elsewhere": 6.0,',
     "NO bar — its null exceeds the signal", None),
    ("an unknown scheme falls back to a default bar",
     '    return (None, "unknown control scheme %r — a bar cannot be chosen for a "',
     '    return (6.0, "unknown control scheme %r — a bar cannot be chosen for a "',
     "gets no bar and says why", None),
    ("a measurement with no supportable bar is given a verdict anyway",
     '                _rec["region_verdict"] = "UNVALIDATED"',
     '                _rec["region_verdict"] = "INERT"',
     "records UNVALIDATED", None),
    ("the INERT verdict fires per placement again, before the population exists",
     '        if _chg is False and _rec.get("mode") == "region":',
     '        if False:',
     "DEFERRED to the end of the run", None),
    ("a held INERT is reported as a defect regardless of separation",
     '        if _sep == "SEPARATES":', '        if True:',
     "ONLY where the bar separates", None),
