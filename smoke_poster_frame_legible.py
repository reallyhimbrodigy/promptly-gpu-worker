...
def main():
    ...
    # A CHECK OVER AN EMPTY POPULATION ASSERTS NOTHING — BUT AN EMPTY POPULATION
    # HAS TWO CAUSES AND THIS USED TO COLLAPSE THEM.
    #
    # EMPTY BECAUSE THE READER BROKE is a defect and must fail. EMPTY BECAUSE
    # EVERY BODY WAS JUST EDITED is the sha binding working exactly as designed:
    # changing a body makes its recorded picture stale, which REMOVES it from the
    # menu until it is re-photographed. Fixing six placeholder strings did that
    # to all seven at once and turned this leg red — a red with no action
    # attached for this lane, because a component awaiting a picture cannot be
    # fixed by committing differently. Only the rig that re-shoots it can.
    #
    # That is the same argument Builder 1 made about L6 and it lands the same
    # way: disclose the state, fail only when the accessor CANNOT account for it.
    awaiting = set(ls.get("awaiting_picture") or [])
    have_body = {n for n in ls["renderable"]
                 if os.path.exists(os.path.join(BODIES, n + ".jsx"))}
    explained = bool(menu) or bool(have_body & awaiting)
    leg("L0 menu_bodies_present_or_explained", explained,
        "%d menu component(s) with a body: %s | %d awaiting a picture after a "
        "body change (re-photograph restores them): %s"
        % (len(menu), ", ".join(sorted(menu)) or "none",
           len(have_body & awaiting), sorted(have_body & awaiting) or "none"))
    # L7 IS DELETED AND THIS IS WHY — removing a check needs a better reason
    # than "redundant".
    #
    # I wrote it to assert port/build == port/bodies after a hand-promotion. THE
    # PREMISE WAS WRONG: port/build is GENERATED. emit_zoom_component.mjs splices
    # the velocity cap in at a `// @@VELOCITY_CAP@@` marker, and its own header
    # says nothing in port/build is edited by hand. For a marker-free body the
    # emitter's output happens to equal the source — which is why my `cp` looked
    # right, and why the committed pair was byte-identical. CORRECT BY ACCIDENT.
    #
    # The real invariant is `emit_zoom_component.mjs --check`: build against
    # EMITTER(bodies), for every component INCLUDING the marker-bearing zooms
    # where build != bodies by design. It already exists and is already gated,
    # in smoke_zoom_source_contain L6 built_blobs_in_sync. L7 was a second,
    # weaker copy of a check this lane already had, and a check that two copies
    # agree is a worse answer than not having two copies.
    #
    # The promotion path is `node port/emit_zoom_component.mjs`, never `cp`.
    #
    # Zac's actual point — no leg executed or scope-checked an edited body, which
    # is how a ReferenceError shipped — is answered by smoke_no_use_before_define
    # against both populations, not by comparing two files.

    # L7b TWO REGISTRATION HOMES ARE DECLARED, NOT PRETENDED AWAY. I tried
    # deleting the duplicate and THE GATE STOPPED ME: port/build/StickyNotes.jsx
    # is not dead, the emitter builds it, and `emit --check` went red the moment
    # it vanished. So the honest shape is not "the overlap is empty" — it is
    # "the overlap is exactly this, for this reason, and a NEW one fails".
    #
    # StickyNotes has two hand-written SOURCES: port/bodies (emitted into
    # port/build) and ported_mg (baked into the registry, which is what actually
    # registers it). Two sources for one name is where drift lives, and it is
    # what made both of Builder 1's false positives look plausible.
    ...
    def _walk(o, path=""):
        if isinstance(o, dict):
            if isinstance(o.get("code"), str) and "const Component" in o["code"]:
                _baked.add(str(o.get("name") or o.get("id") or path).split("/")[-1])
            for k, v in o.items():
                _walk(v, path + "/" + str(k))
        elif isinstance(o, list):
            for i, v in enumerate(o):
                _walk(v, path + "[%d]" % i)
    _walk(_json.load(open(os.path.join(HERE, "chatcut_registry_baked.json"),
                          encoding="utf-8")))
    _build = {f[:-4] for f in os.listdir(os.path.join(HERE, "port", "build"))
              if f.endswith(".jsx")}
    KNOWN_TWO_HOME = {"StickyNotes"}
    two_home = sorted(_build & _baked)
    leg("L7b two_registration_homes_are_declared",
        bool(_baked) and set(two_home) <= KNOWN_TWO_HOME,
        "port/build %d, baked %d, in both: %s | UNDECLARED: %s"
        % (len(_build), len(_baked), two_home or "none",
           sorted(set(two_home) - KNOWN_TWO_HOME) or "none"))

    if not menu:
        # Nothing below can assert anything, so stop — but with the reason named
        # and the exit reflecting whether it was accounted for.
        print("0 on the menu with a body — %s"
              % ("all awaiting a picture after a body change, which is the sha "
                 "binding working; re-photograph to restore"
                 if explained else "UNEXPLAINED, treat as a reader bug"))
        # RESPECT THE TALLY. This used to `return 0 if explained else 1` and so
        # walked straight past a FAILING L7 — the smoke printed L7 FAIL and
        # exited 0. An early return that ignores the failures already recorded
        # is a check passing for a reason other than the one it claims, built
        # into the exit path of the check that exists to catch that.
        return 1 if FAILS else (0 if explained else 1)
