...
def main():
    ...
    # L7 port/build IS WHAT REGISTERS; port/bodies IS A TEMPLATE. They are kept
    # in step BY HAND — port/build is a byte-for-byte COPY of port/bodies at
    # promotion time, verified against the committed pair — and nothing enforced
    # it until now.
    #
    # THIS HAS COST A DAY ALREADY. Commit f1c0f17 says it outright: "port/build
    # was stale for a day: the bodies are templates and I registered from the
    # wrong tree". It then cost the EmojiCard defect, because my fix landed in
    # bodies and the pool was registered from build. And it caught me a THIRD
    # time an hour ago: a placeholder-string sweep across six bodies that would
    # have shipped nothing at all, because every one of the seven had drifted.
    #
    # Three times is not a mistake to notice, it is a default to design against.
    # A template and a build that differ are indistinguishable from a template
    # and a build that agree, right up until somebody registers.
    # SCOPED TO THE FAMILIES WHERE build IS A COPY, and the first run taught me
    # why. Over every shared file it flagged eight — DepthPull, FocusWindow,
    # LetterboxPush, SmoothPush, SnapReframe, StagedPush, StepPush, ZoomThrough —
    # and those are ZOOMS AND TRANSITIONS, whose build is GENERATED by
    # emit_zoom_component.mjs with the velocity cap substituted in. For them
    # build != bodies is CORRECT, and a leg demanding equality would convict
    # seven working components: a pattern tight enough to reject a correct
    # implementation is not a check either.
    #
    # The families are read from the accessor rather than listed here, so a
    # component that changes family is covered the day it moves.
    fam = ls["by_family"]
    copied = set(fam.get("text overlay") or []) | set(fam.get("frame composition") or [])
    shared = sorted(n + ".jsx" for n in copied
                    if os.path.isfile(os.path.join(HERE, "port", "bodies", n + ".jsx"))
                    and os.path.isfile(os.path.join(HERE, "port", "build", n + ".jsx")))
    drifted = []
    for n in shared:
        a = open(os.path.join(HERE, "port", "bodies", n), "rb").read()
        b = open(os.path.join(HERE, "port", "build", n), "rb").read()
        if a != b:
            drifted.append(n.replace(".jsx", ""))
    leg("L7 build_matches_bodies", bool(shared) and not drifted,
        "%d shared component(s); DRIFTED (build would register stale code): %s"
        % (len(shared), drifted or "none"))
