...
for label, old, new, phrase in CASES:
    ...
    _mutant = orig.replace(old, new, 1)
    if not _parses(_mutant, label, fails):
        continue
...
if fails:
    ...
    print("\nRED PROOF: PASS — %d app mutations, %d reds"
          % (len(CASES), len(CASES)))
# THE FLOOR, IN THE SECOND OF THE TWO SPELLINGS THE GUARDS ACCEPT:
# `reds and reds == len(CASES)`. `if fails: exit(1)` reports SUCCESS for a
...
sys.exit(0 if (reds and reds == len(CASES) and not fails) else 1)
