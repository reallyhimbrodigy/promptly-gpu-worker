...
Runs Practical-RIFE 4.18 frame interpolation on a CUDA-capable GPU to
produce truly motion-compensated intermediate frames — much higher
quality than FFmpeg's `minterpolate` filter and dramatically faster
on the H100 GPU.
...
Requires:
  ...
  * /opt/rife layout:
      /opt/rife/model/warplayer.py    (from Practical-RIFE git repo)
      /opt/rife/model/loss.py         (from Practical-RIFE git repo)
      /opt/rife/train_log/RIFE_HDv3.py    (from RIFE 4.18 archive)
      /opt/rife/train_log/IFNet_HDv3.py   (from RIFE 4.18 archive)
      /opt/rife/train_log/refine.py       (from RIFE 4.18 archive)
      /opt/rife/train_log/flownet.pkl     (from RIFE 4.18 archive)
  * ffmpeg with libx264 + audio codec available

Verified against RIFE 4.18 weights (~22MB flownet.pkl, ~10M params).
"""
import argparse
import os
import subprocess
...
def _probe_video(path):
    """Return (width, height, src_fps) or raise."""
    out = subprocess.run(
        [
            "ffprobe", "-v", "error",
            "-select_streams", "v:0",
...
def _load_rife_model(rife_dir, device):
    """Load Practical-RIFE Model from rife_dir/train_log/.

    Adds rife_dir to sys.path so `model.warplayer`, `model.loss`, and
    `train_log.RIFE_HDv3` resolve correctly.
    """
...
def main():
    ...
    ap.add_argument(
        ...
        action="store_true",
        default=True,
        help="(default true) Require CUDA. Pass --no-require-cuda to allow CPU "
             "fallback (only useful for tiny test videos in CI).",
    )
    ap.add_argument("--no-require-cuda", dest="require_cuda", action="store_false")
    args = ap.parse_args()
    ...
    if int_mult < 2:
        raise RuntimeError(
            f"RIFE only used for upscaling; target {target_fps} <= src "
            f"{src_fps:.3f}. Use plain `ffmpeg fps={target_fps}` instead."
        )

    if args.require_cuda and not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA not available. RIFE requires a GPU; this build expected an "
            "H100. Check container nvidia driver mount and torch CUDA wheel."
        )
    ...
    print(
        ...
        f"({int_mult}x multiplier) on {device}",
        flush=True,
    )
    ...
    # intact and we can crop back precisely on the way out.
    ...
    def to_tensor(frame_np):
        """rgb24 numpy -> padded float tensor on device."""
    ...
    try:
        while True:
            ...
            if len(raw) < frame_bytes:
                break
            n_input += 1
            # np.frombuffer returns a read-only view; copy makes the
            # tensor conversion warning-free and the buffer mutable for
            # later consumers (e.g. encoder.stdin.write).
            curr_np = np.frombuffer(raw, dtype=np.uint8).reshape(height, width, 3).copy()
