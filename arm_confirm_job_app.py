...
def presign_and_insert(job_id: str) -> dict:
    ...
    sb.table("video_jobs").insert({
        "id": job_id, "status": "queued", "video_url": SRC_KEY,
...
def main(dry: bool = True, read_only: bool = False, job_id: str = ""):
    ...
    jid = str(uuid.uuid4())
    u = presign_and_insert.remote(jid)
    print(f"  source HEADed, row pre-inserted, job {jid[:8]}")
    if dry:
