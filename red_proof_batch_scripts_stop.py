...
MUTATIONS = [
    ...
    ("a withheld export is delivered to Zac's folder anyway", MP4SC,
     '    for p in (out, os.path.join(DELIVER_DIR, os.path.basename(out)) if DELIVER_DIR else out):',
     '    for p in (out,):',
     "a refusal REMOVES a stale file already sitting in the Reports folder"),
    ("the delivered copy is never re-hashed", MP4SC,
     '        return hashlib.sha256(open(dst, "rb").read()).hexdigest() == sha',
     '        return True',
     "the delivered copy is re-hashed: a file that does not match is refused, and a missing one too"),
