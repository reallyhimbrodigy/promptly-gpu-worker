...
# ── A MUTANT THAT DOES NOT PARSE NEVER RAN ──────────────────────────────────
# Ported in with this harness on the merge, and it lacked the guard this lane's
# smoke_red_proofs_guarded requires. If a mutation produces source Python
# cannot compile, the gate under test dies on the SyntaxError rather than on
# the property, and the leg counts RED while having proved nothing. So every
# mutant is compiled first, and a mutant that will not parse is a FAILURE of
# the harness, not a red.
def _parses(_src, _label, _fails):
    import ast
    try:
        ast.parse(_src)
        return True
    except SyntaxError as _e:
        _fails.append("%s: the MUTANT does not parse (%s) — the gate would "
                      "die on the syntax, not on the property, and this leg "
                      "would count RED having proved nothing" % (_label, _e))
        print("  [BAD MUTANT] %s" % _label)
        return False
...
try:
    ...
    for _label, _old, _new, _phrase in MUTATIONS:
        ...
        if _pristine.count(_old) != 1:
            ...
            continue
        _mutant = _pristine.replace(_old, _new, 1)
        if not _parses(_mutant, _label, fails):
            continue
        _app.write_text(_mutant)
        rc, out = run(_wt)
        _red = rc != 0
        _said = _phrase.lower() in out.lower()
        reds += 1 if (_red and _said) else 0
...
# `reds and reds == len(MUTATIONS)`. `if fails: exit(1)` reports SUCCESS for a
...
sys.exit(0 if (reds and reds == len(MUTATIONS) and not fails) else 1)
