...
def _secret_canonical_values():
    ...
    CANON = {
        ...
        "PROMPTLY_OVERLAY_TAB_BUDGET": "8",  # 48-TAB CONTENTION FIX (Zac 2026-08-02, MEASURED -40%): 32 Chromium tabs across the parallel overlay chunks on a cpu=16 render container = 2x oversubscription; render 303s@32 -> 243s@16 -> 183s@8 on a chunked clip (byte-identical, concurrency only). Also fixes the contention RENDER_FATAL (micro chunk crawling 0.3 fps into the 600s floor, job 2f07c37b). rollback = unset here + secret + redeploy -> 32.
    }
