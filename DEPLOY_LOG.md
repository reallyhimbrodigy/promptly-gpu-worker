...
| **C2 — QUEUED behind W5** | DELIVERY | lane/delivery-2 @ `bd594fb` | export server half DARK (alias route + watermark-at-export v1) + the stuck-job signature becomes a named event. Ships **dark**; the two export flags flip later per DELIVERY's four-step order (see `LAUNCH_DAY.md` §6d). Carries an in-container ffmpeg watermark proof that runs in the gate, so "does the watermark pass work in THIS container" is answered on every deploy, never by the first paying customer. | queued |
