...
# a launch that never started a container must not be waited on forever: the idle loop below is bounded at 25 min
while [ "$(modal app list 2>/dev/null | /usr/bin/grep -i 'chatcut' | /usr/bin/grep -ci 'running\|ephemeral')" != "0" ]; do sleep 15; [ $(( $(date +%s) - T0 )) -gt 1500 ] && { echo "STOP: no idle after 1500s"; break; }; done
